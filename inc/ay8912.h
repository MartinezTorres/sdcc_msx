...
static void ayemu_init(ayemu_ay_t *ay);

static void ayemu_reset(ayemu_ay_t *ay);

static int ayemu_set_chip_type(ayemu_ay_t *ay, ayemu_chip_t chip, int *custom_table);

static void ayemu_set_chip_freq(ayemu_ay_t *ay, int chipfreq);

static int ayemu_set_stereo(ayemu_ay_t *ay, ayemu_stereo_t stereo, int *custom_eq);

static int ayemu_set_sound_format (ayemu_ay_t *ay, int freq, int chans, int bits);

static void ayemu_set_regs (ayemu_ay_t *ay, ayemu_ay_reg_frame_t regs);

static void* ayemu_gen_sound (ayemu_ay_t *ay, void *buf, size_t bufsize);

static char *ayemu_err;
static const int MAGIC1 = 0xcdef;	/* for check ayemu_t structure inited */
...
static int check_magic(ayemu_ay_t *ay)
{
  if (ay->magic == MAGIC1)
    return 1;
  fprintf(stderr, "libayemu: passed pointer to uninitialized ayemu_ay_t structure\n");
  return 0;
}
...
/** Set chip type. */
static int ayemu_set_chip_type(ayemu_ay_t *ay, ayemu_chip_t type, int *custom_table)
{
if (!check_magic(ay))
		return 0;

  if (!(type == AYEMU_AY_CUSTOM || type == AYEMU_YM_CUSTOM) && custom_table != NULL) {
    ayemu_err = (char *)"For non-custom chip type 'custom_table' param must be NULL";
    return 0;
  }
  ...
  case AYEMU_YM_LOG:
  default:
    ayemu_err = (char *)"Incorrect chip type";
    return 0;
  }
...
static int ayemu_set_sound_format (ayemu_ay_t *ay, int freq, int chans, int bits)
{
  ...
  if (!(bits == 16 || bits == 8)) {
    ayemu_err = (char *)"Incorrect bits value";
    return 0;
  }
  else if (!(chans == 1 || chans == 2)) {
    ayemu_err = (char *)"Incorrect number of channels";
    return 0;
  }
  else if (freq < 50) {
    ayemu_err = (char *)"Incorrect output sound freq";
    return 0;
  }
  else {
    ay->sndfmt.freq = freq;
 ...
 */
static int ayemu_set_stereo(ayemu_ay_t *ay, ayemu_stereo_t stereo_type, int *custom_eq)
{
  int i;
  int chip;
  ...
  if (stereo_type != AYEMU_STEREO_CUSTOM && custom_eq != NULL) {
    ayemu_err = (char *)"Stereo type not custom, 'custom_eq' parametr must be NULL";
    return 0;
  }
  ...
  case AYEMU_STEREO_CUSTOM:
    ...
    break;
  default:
    ayemu_err = (char *)"Incorrect stereo type";
    return 0;
  }
 ...
 */
static void *ayemu_gen_sound(ayemu_ay_t *ay, void *buff, size_t sound_bufsize)
{
  int mix_l, mix_r;
  int tmpvol;
  int m;
