...
auto compyuv = [](cv::Vec3b p0, cv::Vec3b p2) {
	...
	bool isGray = std::abs(p2[2]-p2[1])<10 and std::abs(p2[0]-p2[1])<10;
	
	p0 = bgr2yuv(p0);
	p2 = bgr2yuv(p2);
	
	double factor = 1;
	if (p2[0]>235) factor = 40;
	if (p2[0]<20) factor = 40;
	
	double color2gray_ratio = 4;
	if (isGray) color2gray_ratio = 50;
	

	
	if (1) return	factor*((p0[0]-p2[0])*(p0[0]-p2[0]) +
...
/*			{   0,    0,    0},
	...
	std::clog << "Gray Pallette 0: " << std::endl;
	...
	std::clog << "Gray Pallette 1: " << std::endl;
	std::clog <<  ts.add(cv::Vec3b(202,  202,  202), {   0,    0,    0}) << std::endl;
	std::clog <<  ts.add(cv::Vec3b(202,  202,  202), { 202,  202,  202}) << std::endl;
	std::clog <<  ts.add(cv::Vec3b(255,  255,  255), {   0,    0,    0}) << std::endl;
	std::clog <<  ts.add(cv::Vec3b(255,  255,  255), { 202,  202,  202}) << std::endl;

	std::clog << "Gray Pallette 2: " << std::endl;
	std::clog <<  ts.add(cv::Vec3b(202,  202,  202), { 202,  202,  202}) << std::endl;
	std::clog <<  ts.add(cv::Vec3b(202,  202,  202), { 255,  255,  255}) << std::endl;
	std::clog <<  ts.add(cv::Vec3b(255,  255,  255), { 202,  202,  202}) << std::endl;
...
//		imgd = imgd*(200./255.)+25;
		...
		img = cv::Mat3b(imgd(cv::Rect(imgd.cols/5,0*imgd.rows/5,3*imgd.cols/5,3*imgd.rows/5)).clone());
...
//		img = cv::Mat3b( cv::Mat3f(img)*(200./255.)+25 );

		for (auto &c : img) if (c[1]>231) c[1]=231;
