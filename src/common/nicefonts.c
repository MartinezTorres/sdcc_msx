...
void initRenderedText(uint8_t *slim, uint8_t *bold, uint8_t freeTiles[256], const uint8_t *str, const TFont font, uint8_t padding) {
	...
	uint8_t maxDisplacement = 32-8;
	uint32_t workAreaSlim[8];
	uint32_t workAreaBold[8];
	INIT(workAreaSlim,0);
	INIT(workAreaThick,0);
	...
	while (*str) {
		...
		while (TRUE) {
			uint8_t i;
			for (i=0; i<8; i++) 
				if ((workArea[i] & (((uint32_t)font[c][i]) << maxDisplacement)) != 0) 
					goto out;
			maxDisplacement++;
		}
		out:
		...
		if (maxDisplacement<10) {
			...
			*slim++ = slimwriteShape(workAreaSlim, freeTiles);
			*bold++ = boldwriteShape(workAreaBold, freeTiles);
			maxDisplacement += 8;
		}
	}
