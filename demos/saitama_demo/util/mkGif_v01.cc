...
//		cv::resize(img, img, cv::Size(), scale, scale, cv::INTER_AREA);
	...
	if (1) {
		...
		for (int k=0; k<int(images.size()); k++) {
			...
			source_main << "    " << "fast_load_page_c(SEGMENT_C("<<name<<frameNr<<")); " << name << frameNr << "();" << std::endl;
			...
			for (int l=0; l<img3b.rows; l+=64) {
				...
				if (P0.size()) SP0 = writeDataBlock(P0.begin(), P0.end(), path, name+"%05d"+"_%05d"+"_P0",k,l/64);
				if (P1.size()) SP1 = writeDataBlock(P1.begin(), P1.end(), path, name+"%05d"+"_%05d"+"_P1",k,l/64);
				if (C0.size()) SC0 = writeDataBlock(C0.begin(), C0.end(), path, name+"%05d"+"_%05d"+"_C0",k,l/64);
				if (C1.size()) SC1 = writeDataBlock(C1.begin(), C1.end(), path, name+"%05d"+"_%05d"+"_C1",k,l/64);
...
//				if (C1.size()) file_frame_main << "    fast_load_page_d(SEGMENT_D(" << SC1 << ")); gif_v01_addC1_section" << (l/64) << "(" << SC1 << ", " << C1.size()/8 << ");" << std::endl;
				...
				if (P0.size()) { file_frame_main << "    fast_load_page_d(SEGMENT_D("<<SP0<<")); gif_v01_copy_vpn_x8( MODE2_ADDRESS_PG+(128*8*(2*"<<(l/64)<< "+0))+8*" << s0 << " , " << SP0 << ", " << e0 << "); ";  if (e1) file_frame_main << " gif_v01_copy_vpn_x8( MODE2_ADDRESS_PG+(128*8*(2*"<<(l/64)<< "+0)), " << SP0 << "+8*" << e0 << ", " << e1 << ");"; file_frame_main << std::endl; }
				if (P1.size()) { file_frame_main << "    fast_load_page_d(SEGMENT_D("<<SP1<<")); gif_v01_copy_vpn_x8( MODE2_ADDRESS_PG+(128*8*(2*"<<(l/64)<< "+1))+8*" << s0 << " , " << SP1 << ", " << e0 << "); ";  if (e1) file_frame_main << " gif_v01_copy_vpn_x8( MODE2_ADDRESS_PG+(128*8*(2*"<<(l/64)<< "+1)), " << SP1 << "+8*" << e0 << ", " << e1 << ");"; file_frame_main << std::endl; }
				if (C0.size()) { file_frame_main << "    fast_load_page_d(SEGMENT_D("<<SC0<<")); gif_v01_copy_vpn_x8( MODE2_ADDRESS_CT+(128*8*(2*"<<(l/64)<< "+0))+8*" << s0 << " , " << SC0 << ", " << e0 << "); ";  if (e1) file_frame_main << " gif_v01_copy_vpn_x8( MODE2_ADDRESS_CT+(128*8*(2*"<<(l/64)<< "+0)), " << SC0 << "+8*" << e0 << ", " << e1 << ");"; file_frame_main << std::endl; }
				if (C1.size()) { file_frame_main << "    fast_load_page_d(SEGMENT_D("<<SC1<<")); gif_v01_copy_vpn_x8( MODE2_ADDRESS_CT+(128*8*(2*"<<(l/64)<< "+1))+8*" << s0 << " , " << SC1 << ", " << e0 << "); ";  if (e1) file_frame_main << " gif_v01_copy_vpn_x8( MODE2_ADDRESS_CT+(128*8*(2*"<<(l/64)<< "+1)), " << SC1 << "+8*" << e0 << ", " << e1 << ");"; file_frame_main << std::endl; }
			...
			for (int l=0; l<img3b.rows; l+=64) {
				...
				if (PN.size()) SPN = writeDataBlock(PN.begin(), PN.end(), path, name+"%05d"+"_%05d"+"_PN",k,l/64);
				if (PN.size() == 256 ) file_frame_main << "    fast_load_page_d(SEGMENT_D(" << SPN << ")); gif_v01_addPN_section" << (l/64) << "_256(" << SPN << ");" << std::endl;
				else if (PN.size()) file_frame_main << "    fast_load_page_d(SEGMENT_D(" << SPN << ")); gif_v01_addPN_section" << (l/64) << "(" << SPN << ", " <<skip << ", " << PN.size() << ");" << std::endl;
			}
